...
namespace Functions.Helpers
{
    public static class RetryHelper
    {
        ...
        {
            return (exception.InnerException is FlurlHttpException flurlHttpException && // Handle rate limits (happens if we got blocked by rate limits)
                    flurlHttpException.Call.HttpStatus == (HttpStatusCode) 429)
                   || (exception.InnerException is SocketException socketException && // Handle timeout (happens if we got delayed by rate limits)
